...
using (var wb =  excelApp?.Proxy.Workbooks.Add() as ComDispatchProxy<Excel.Workbook>)
using (var ws =  wb?.Proxy.Sheets[1] as ComDispatchProxy<Excel.Worksheet>)
using (var targetRange = ws?.Proxy.Range["A1:A10"] as ComDispatchProxy<Excel.Range>)

if (excelApp is not null && targetRange is not null)
{
    ...
    targetRange.Proxy.Columns.AutoFit();                // 列幅を自動調整
}
